...
public class FormattedJavascriptChangeSetReaderTest {
    ...
    private List<ChangeSet> parse(String fileName) throws URISyntaxException {
        return parse(null, fileName);
    }

    private List<ChangeSet> parse(Charset charset, String fileName) throws URISyntaxException {
        FormattedJavascriptChangeSetReader reader = charset != null ? new FormattedJavascriptChangeSetReader(charset)
                : new FormattedJavascriptChangeSetReader();
        File file = Paths.get(this.getClass().getResource(fileName).toURI()).toFile();
        List<ChangeSet> changeSets = reader.getChangeSets(file);
        return changeSets;
    }

    private void assertChangeSetEquals(ChangeSet actual, String expectedAuthor, String expectedChangeId,
            boolean expectedRunAlways, String expectedFile, String expectedBody) {
